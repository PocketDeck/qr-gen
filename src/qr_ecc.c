...
static void poly_div(uint8_t *dividend, size_t dividend_len, 
                    const uint8_t *divisor, size_t divisor_len) {
...
int qr_ec_encode(qr_ec *ec, const uint8_t *data, uint8_t *ecc) {
    ...
    poly_div(msg, ec->data_length + ec->ecc_length, 
            ec->generator, ec->ecc_length + 1);
